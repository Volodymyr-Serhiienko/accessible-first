...
- Can keep header, navigation, or AppShell `beforeOutlet` content in normal flow, sticky position, or fixed position.
- Measures active chrome regions and sets scroll-safe offsets so route changes and focus routes do not hide content under sticky chrome.
...
- `chrome` - page chrome positioning. Accepts `"normal"`, `"sticky"`, `"fixed"`, or `{ header, navigation, beforeOutlet, topOffset, zIndex }`. Defaults to normal flow.

## Chrome Modes

`normal` keeps the selected region in document flow.

`sticky` keeps the selected region visible after it reaches the top of the viewport. It is the safest first choice for app headers, navigation, breadcrumbs, and route helper bars because it does not remove the region from layout flow.
...
PageLayout measures header, navigation, and before-outlet heights with `ResizeObserver` when available. It exposes these CSS variables on the page root:
...
Future layout work may add reveal-on-scroll behavior. That mode should remain semantic-neutral and should not add landmarks or focus targets by itself.
...
Sticky breadcrumbs or context bars should not replace page headings or navigation landmarks. They are orientation aids, not the only way to understand the current screen.
...
- Sticky header/navigation/breadcrumbs remain visible without creating duplicate landmarks.
- Route changes scroll to the beginning of the visible screen content without hiding it under sticky chrome.
