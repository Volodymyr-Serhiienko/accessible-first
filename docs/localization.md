...
For route-owned application text, use `createLocalizedAppRouteText()` so navigation, search, breadcrumbs, announcements, metadata, and diagnostics can share the same localized route resolvers. Larger apps can keep this wiring in a small app-owned `routeText.ts` file beside `routes.ts`.
