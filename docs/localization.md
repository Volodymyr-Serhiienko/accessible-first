...
For route-owned application text, use `createLocalizedAppRouteText()` so navigation, search, breadcrumbs, announcements, metadata, and diagnostics can share the same localized route resolvers. Larger apps can keep this wiring in a small app-owned `routeText.ts` file beside `routes.ts`. Pass the `routeText` bundle to public app templates so they can merge route text into metadata/diagnostics defaults and, for hash SPAs, use `getLoadedAnnouncement` unless the router overrides it. Pass `routeLoadedAnnouncementKey` when route-change announcements should use one localized app-owned phrase such as `{title} loaded`; use per-route `localeKeys.loadedAnnouncement` only for screens that need a different spoken phrase.
